function [SBL_xhat,err] = log_SBL_NF(A,y,N,sig2e,Tau_p,Hsf,dft_h)
max_iter_sbl=50;M=size(y,2);
rho_0=.01;
lambda=1/M;
...
u=zeros(N,N);
c=zeros(N,N);
gamma=real(diag(Gamma(:,:,1)));%B=eye(N,N);
w=ones(N,N);l_k=1;
...
%         end
    elseif t>=sbl_max_iter
        ...
        for i=1:N
           % kappa=100;
            for j=1:i-1
                ...
                mu=min(.01*t*M,2);
            w(i,j)=exp(-abs(log(gamma(i))-log(gamma(j)))^2);
        ...
        for tep=1:5
            ...
            for i=1:N
                for j=1:i-1
                    gmbar=real(w(i,j)*(log(gamma(i))-log(gamma(j)))-u(i,j)/rho);
                    c(i,j)=real(sign(gmbar)*max(0,abs(gmbar)-lambda/rho)); %%B(i,j)*
                    c(j,i)=-c(i,j);
                    u(i,j)=real(u(i,j)+rho*(c(i,j)-w(i,j)*(log(gamma(i))-log(gamma(j)))));
            ...
            for i=1:N
                ...
                gm_bar=sum((c(i,se)).'+(w(i,se)).'.*log(bdiag(se))+u(i,se).'/rho);
                gm_tilde=sum((c(se2,i))-(w(se2,i)).*log(bdiag(se2))+u(se2,i)/rho);              % Nhat=length(se)+length(se2);
