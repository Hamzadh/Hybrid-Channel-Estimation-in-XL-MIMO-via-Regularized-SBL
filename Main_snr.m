...
L = 8; % number of all paths
gamma=0.5; 
Lf =ceil(L*gamma); % number of paths for far-field 
Ln = ceil(L*(1-gamma)); % number of paths for near-field
M = N/4; % number of pilot overhead
...
nb_of_best_samples=ceil(.9*sample)
...
nmse_proposed=10*log10(nmse_proposed)
...
figure('color',[1,1,1]); 
ha=gca;
plot(SNR_dB,nmse_proposed,'-.s',SNR_dB,nmse_burst,'--^',SNR_dB,nmse_practical,'--',SNR_dB,nmse_omp_dft,'<-',SNR_dB,nmse_omp_qua,'>-',...
        SNR_dB,nmse_homp,'rs-','linewidth',2.5);
...
xlabel('Pilot length $T$','Interpreter','latex','FontSize',14)
