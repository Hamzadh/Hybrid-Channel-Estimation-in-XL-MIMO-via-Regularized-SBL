...
%%% system parameters
N = 2*256; % number of beams (transmit antennas)
L = 8; % number of all paths

alpha_vec=0:1/L:1
len = length(alpha_vec);


M = 128; % number of pilot overhead
...
xlabel('\bf $ \gamma$','Interpreter','latex','FontSize',14)
xlim([alpha_vec(1) alpha_vec(end)])
xticks(alpha_vec)
